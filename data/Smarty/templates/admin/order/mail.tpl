...
<div id="order" class="contents-main">
    ...
    <table class="form">
        ...
        <!--{assign var=key value="template_id"}-->
        <!--{if $arrForm[$key].value == 8}-->
        <!--{* 2011.04.27 メールに記載するお届け先選択 *}-->
        <tr>
          <th>お届け先<br /></th>
          <td>
                <span class="attention"><!--{$arrErr[$key]}--></span>
                <select name="shipping_addr" style="<!--{$arrErr[$key]|sfGetErrorColor}-->">
                <option value="" selected="selected">全件選択</option>
                <!--{html_options options=$arrShippingList selected=$arrForm[$key].value|h}-->
                </select>
            </td>
        </tr>
        <!--{/if}-->
