...
*}-->
<!--{*
 * edit.tpl - 受注情報編集(Veritrans3Gによる上書き 2.11.2 origin)
 *
 * LICENSE: （ライセンスに関する情報）
 *
 * @category    Veritrans
 ...
 * @version     $Id: edit.tpl 175 2012-07-25 05:44:03Z hira $
 * @link        http://www.veritrans.co.jp/3gps
 * @see         data/Smarty/templates/admin/order/edit.tpl
 *}-->
        ...
        <table class="form">
            <tr>
                ...
                <td>
                    ...
                    <input type="text" name="<!--{$key1}-->[<!--{$shipping_index}-->]" value="<!--{$arrShipping[$key1]|h}-->" maxlength="<!--{$arrForm[$key1].length}-->" style="<!--{$arrErr[$key1][$shipping_index]|sfGetErrorColor}-->" size="20" class="box20" />
                </td>
            </tr>
            <tr>
                <th>お名前(フリガナ)</th>
            ...
            <tr>
                <th>宅配ボックス</th>
                <td>
                    <!--{assign var=key1 value="box_flg"}-->
                    <!--{html_radios name=$key1 options=$arrBoxFlg selected=$arrShipping[$key1]|default separator='&nbsp;&nbsp;'}-->
                </td>
            </tr>
...
<!--▼ Veritrans 3G Module -->
<!--{include file=$smarty.const.MDL_SBIVT3G_TPL_PATH|cat:"admin/order/edit_payment.tpl"}-->
<!--▲ Veritrans 3G Module -->
