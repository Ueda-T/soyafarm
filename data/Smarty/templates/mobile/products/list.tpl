...
<font color="#ff6600">
<!--★価格★-->
<!--{if strlen($tpl_customer_kbn) == null || $tpl_customer_kbn == $smarty.const.CUSTOMER_KBN_NORMAL}-->￥<!--{if $arrProduct.price01_min == $arrProduct.price01_max}--><!--{$arrProduct.price01_min|sfCalcIncTax:$arrSiteInfo.tax:$arrSiteInfo.tax_rule|number_format}--><!--{else}--><!--{$arrProduct.price01_min|sfCalcIncTax:$arrSiteInfo.tax:$arrSiteInfo.tax_rule|number_format}-->～<!--{$arrProduct.price01_max|sfCalcIncTax:$arrSiteInfo.tax:$arrSiteInfo.tax_rule|number_format}--><!--{/if}-->(税込)
<!--★社員価格★--><!--{elseif $tpl_customer_kbn == $smarty.const.CUSTOMER_KBN_EMPLOYEE }-->￥<!--{if $arrProduct.price02_min == $arrProduct.price02_max}--><!--{$arrProduct.price02_min|sfCalcIncTax:$arrSiteInfo.tax:$arrSiteInfo.tax_rule|number_format}--><!--{else}--><!--{$arrProduct.price02_min|sfCalcIncTax:$arrSiteInfo.tax:$arrSiteInfo.tax_rule|number_format}-->～<!--{$arrProduct.price02_max|sfCalcIncTax:$arrSiteInfo.tax:$arrSiteInfo.tax_rule|number_format}--><!--{/if}-->(税込)<!--{/if}-->
</font>
</td>
</tr>
...
<tr>
<td align="center" colspan="2"><font size="-1" color="#ffffff">

<!--{* ▼前へと次へのリンクが両方存在している場合*}-->
<!--{if $objNavi->arrPagenavi.beforeUrlParam|strlen > 0 &&
        $objNavi->arrPagenavi.nextUrlParam|strlen > 0 }-->
<a href="?mode=<!--{$objNavi->arrPagenavi.mode|h}-->&<!--{$objNavi->arrPagenavi.beforeUrlParam}-->&name=<!--{$arrSearchData.name|h}-->" accesskey="*"><font color="#ffffff">≪[*]前</font></a>
<a href="?mode=<!--{$objNavi->arrPagenavi.mode|h}-->&<!--{$objNavi->arrPagenavi.nextUrlParam}-->&name=<!--{$arrSearchData.name|h}-->" accesskey="#"><font color="#ffffff">[#]次≫</font></a><br>

<!--{* ▼前へのみ*}-->
<!--{elseif $objNavi->arrPagenavi.beforeUrlParam|strlen > 0}-->
<a href="?mode=<!--{$objNavi->arrPagenavi.mode|h}-->&<!--{$objNavi->arrPagenavi.beforeUrlParam}-->&name=<!--{$arrSearchData.name|h}-->" accesskey="*"><font color="#ffffff">≪[*]前</font></a><br>

<!--{* ▼次へのみ*}-->
<!--{elseif $objNavi->arrPagenavi.nextUrlParam|strlen > 0}-->
<a href="?mode=<!--{$objNavi->arrPagenavi.mode|h}-->&<!--{$objNavi->arrPagenavi.nextUrlParam}-->&name=<!--{$arrSearchData.name|h}-->" accesskey="#"><font color="#ffffff">[#]次≫</font></a><br>
<!--{/if}-->

<!--{* ▼ #343 ○件中 N～Y件表示 *}-->
<!--{$tpl_from_no}-->-<!--{$tpl_to_no}-->件/全<!--{$tpl_linemax}-->件
<!--{* ▲ #343 ○件中 N～Y件表示 *}-->
</font></td>
</tr>
</table>
