...
class SC_SendMail {
    ...
    function setSubject($subject) {
        // 指定コードへエンコーディング
        $this->subject = mb_encode_mimeheader($subject, MAIL_ENCODING, 'B', "\n");
    ...
    function setBody($body) {
        ...
        if (strcasecmp(MAIL_ENCODING, CHAR_CODE) != 0) {
            $this->body = mb_convert_encoding($body, MAIL_ENCODING, CHAR_CODE);
        } else {
            // エンコードしない
            $this->body = $body;
        }
    ...
    function getNameAddress($name, $mail_address) {
            if($name != "") {
                ...
                $_name = ereg_replace(">","＞", $_name);
                // 指定コードへエンコーディング
                $_name = mb_encode_mimeheader($_name, MAIL_ENCODING, 'B', "\n");
    ...
    */
    ...
    function getTEXTHeader() {
        ...
        // mimeエンコーディングを取得
        $encoding = mb_preferred_mime_name(MAIL_ENCODING);
        $arrHeader['Content-Type'] = "text/plain; charset=\"$encoding\"";
    ...
    function getHTMLHeader() {
        ...
        // mimeエンコーディングを取得
        $encoding = mb_preferred_mime_name(MAIL_ENCODING);
        $arrHeader['Content-Type'] = "text/html; charset=\"$encoding\"";
        $arrHeader['Content-Transfer-Encoding'] = $encoding;
        return $arrHeader;
    }
