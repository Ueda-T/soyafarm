...
class LC_Page_Products_Detail extends LC_Page_Ex {
    ...
    function init() {
        ...
        $this->arrSTATUS = $objDb->sfGetProductStatusList();
	...
	global $CLICK_ANALYZER_PRODUCTS;
	...
	$productCode = $this->objFormParam->getValue("product_code");
	// 商品コードにて画面遷移時
	if ($productCode) {
	    // 商品コードより商品ID取得
	    $objChkProduct = new SC_Product_Ex();
	    $arrChkProduct = 
		$objChkProduct->getProductsClassProductCode($productCode);
	    // 商品IDセット
	    if ($arrChkProduct["product_id"]) {
		$this->objFormParam->setValue("product_id"
					      , $arrChkProduct["product_id"]);
	    }
	    // 規格IDセット
	    if ($arrChkProduct["product_class_id"]) {
		$this->objFormParam->setValue("product_class_id"
					      , $arrChkProduct["product_class_id"]);
	    }
	}
	...
	if (isset($CLICK_ANALYZER_PRODUCTS[$product_id])) {
	    $this->tpl_clickAnalyzer = $CLICK_ANALYZER_PRODUCTS[$product_id];
    }

    // 計測タグセット
    $this->lfSetTag($this,$productCode);
	...
	if ($_GET[CAMPAIGN_PARAM_STR]) {
        ...
        switch ($this->mode) {
            case 'cart':
                ...
                // #237 モバイルのページ遷移に対応
                if(SC_Display_Ex::detectDevice() == DEVICE_TYPE_MOBILE) {
                    // 商品詳細画面の「定期購入する」クリック時
                    if (isset($_REQUEST["select_regular"])) {
                        $this->objFormParam->setValue('regular_flg', REGULAR_PURCHASE_FLG_ON);
                    }

                    $product_class_id = $this->objFormParam->getValue('product_class_id');

                    // #261 商品一覧からの遷移に対応
                    if ($this->tpl_classcat_find1 === false
                        && $this->tpl_classcat_find2 === false
                        && empty($product_class_id)) {

                        $product_class_id = 
                            $objProduct->getProductClassIdByProductId
                                ($product_id);
                        $this->objFormParam->setValue
                ...
                if (count($this->arrErr) == 0) {
                    ...
                    // 定期購入フラグ
                    $regular_flg = $this->objFormParam->getValue('regular_flg');
                    if (empty($regular_flg)) {
        ...
        if(SC_Display_Ex::detectDevice() == DEVICE_TYPE_MOBILE) {
            ...
            if ($this->tpl_classcat_find1 === false
                ...
                $this->tpl_mode = 'select';
            }

            // mobile用（定期購入ボタンでの遷移かどうかを判定）
            if (isset($_REQUEST['select_regular'])) {
                $this->tpl_regular_flg = REGULAR_PURCHASE_FLG_ON;
                $this->objFormParam->setValue('regular_flg', REGULAR_PURCHASE_FLG_ON);
                $this->arrForm['regular_flg'] = REGULAR_PURCHASE_FLG_ON;
            }
        ...
        // メタタグ
        $this->tpl_metatag = $this->arrProduct['metatag'];

        // 関連カテゴリを取得
        $this->arrRelativeCat = SC_Helper_DB_Ex::sfGetMultiCatTree($product_id);

        // 商品ステータスを取得
        $this->productStatus = $objProduct->getProductStatus
            ($product_id, SC_Display_Ex::detectDevice());
    ...
    function lfCheckProductId($admin_mode,$product_id) {
        ...
        if (isset($admin_mode) && $admin_mode == 'on') {
            ...
            $where .=<<<EOF
   AND (DATE_FORMAT(disp_start_date, '%Y%m%d') <= DATE_FORMAT(now(), '%Y%m%d')
       OR disp_start_date IS NULL)
EOF;
        }

        if (!SC_Utils_Ex::sfIsInt($product_id) ||
	    SC_Utils_Ex::sfIsZeroFilling($product_id) ||
	    !SC_Helper_DB_Ex::sfIsRecord('dtb_products', 'product_id',
					 (array)$product_id, $where)) {
    ...
    function lfInitParam(&$objFormParam) {
        ...
        $objFormParam->addParam("定期購入フラグ", "regular_flg", INT_LEN, 'n', array("NUM_CHECK", "MAX_LENGTH_CHECK"));
        ...
        $objFormParam->addParam("商品コード", "product_code", INT_LEN, 'a', array("MAX_LENGTH_CHECK"));
        // 値の取得
    ...
    function lfPreGetRecommendProducts($product_id) {
        ...
        $sql =<<<EOF
SELECT
    R.recommend_product_id,
    R.comment
FROM
    dtb_recommend_products R
INNER JOIN 
    dtb_products P
    ON P.product_id = R.recommend_product_id
WHERE
...
EOF;
        ...
        $arrRecommendData = $objQuery->getAll($sql);
...
EOF;
     ...
     *
     */
    function lfSetTag(&$thisPage, $product_code) {
        global $BLADE_TAG_PRODUCT;
        global $MARKETONE_TAG_PRODUCT;
        ...
        if (!empty($tag_tpl_dir)) {
            // BLADEタグ
            if (isset($BLADE_TAG_PRODUCT[$product_code]) &&
                $BLADE_TAG_PRODUCT[$product_code] === true) {
                $thisPage->tpl_tag_blade =
                    $objTagView->fetch($tag_tpl_dir . "blade.tpl");
            }

            // MarketOneタグ
            if (isset($MARKETONE_TAG_PRODUCT[$product_code]) &&
                $MARKETONE_TAG_PRODUCT[$product_code] === true) {
