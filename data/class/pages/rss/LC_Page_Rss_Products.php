 ...
 * @version $Id: LC_Page_Rss_Products.php 91 2012-04-11 04:39:04Z hira $
 */
class LC_Page_Rss_Products extends LC_Page_Ex {
    ...
    function lfGetProductsDetailData($mode, $product_id) {
        ...
        foreach ($arrProduct as $key => $val) {
            //販売価格を税込みに編集
            $arrProduct[$key]['price01'] = SC_Helper_DB_Ex::sfCalcIncTax($arrProduct[$key]['price01']);
            $arrProduct[$key]['price02'] = SC_Helper_DB_Ex::sfCalcIncTax($arrProduct[$key]['price02']);
            // 画像ファイルのURLセット
            // 2013.12.03 add 案内画像追加
            if ( file_exists(IMAGE_SAVE_REALDIR . $arrProduct[$key]['guide_image']) ) {
                $dir = IMAGE_SAVE_RSS_URL;
            } else {
                $dir = IMAGE_TEMP_RSS_URL;
            }
            $arrProduct[$key]['guide_image'] = $dir . $arrProduct[$key]['guide_image'];
            if ( file_exists(IMAGE_SAVE_REALDIR . $arrProduct[$key]['guide_image_teiki']) ) {
                $dir = IMAGE_SAVE_RSS_URL;
            } else {
                $dir = IMAGE_TEMP_RSS_URL;
            }
            $arrProduct[$key]['guide_image_teiki'] = $dir . $arrProduct[$key]['guide_image_teiki'];
...
SELECT
  T1.category_id,
  T1.rank AS product_rank,
  T2.rank AS category_rank
FROM
  dtb_product_categories AS T1
  LEFT JOIN dtb_category AS T2
    ON T1.category_id = T2.category_id
WHERE
    product_id = $product_id
EOF;

            $arrCategory = $objQuery->getAll($sql);
