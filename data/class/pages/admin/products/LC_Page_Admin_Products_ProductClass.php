 ...
 * @version $Id: LC_Page_Admin_Products_ProductClass.php 104 2012-04-24 02:10:35Z hira $
 */
class LC_Page_Admin_Products_ProductClass extends LC_Page_Admin_Ex {
    ...
    function registerProductClass($arrList, $product_id, $total) {
        ...
        for ($i = 0; $i < $total; $i++) {
            ...
            $stock_status_name = SC_Utils_Ex::isBlank($arrList['stock_status_name'][$i]) ? NULL : $arrList['stock_status_name'][$i];
            $teiki_flg = SC_Utils_Ex::isBlank($arrList['teiki_flg'][$i]) ? 0 : $arrList['teiki_flg'][$i];
            $deliv_judgment = SC_Utils_Ex::isBlank($arrList['deliv_judgment'][$i]) ? DELIV_JUDGMENT_DEFAULT_VALUE : $arrList['deliv_judgment'][$i];
            ...
            $arrPC['sale_minimum_number'] = $arrDefault['sale_minimum_number'];
            $arrPC['deliv_fee'] = $arrDefault['deliv_fee'];
            $arrPC['point_rate'] = $arrDefault['point_rate'];
            $arrPC['stock_unlimited'] = $stock_unlimited;
            $arrPC['stock_status_name'] = $arrDefault['stock_status_name'];
            $arrPC['teiki_flg'] = $arrDefault['teiki_flg'];
            $arrPC['deliv_judgment'] = $arrDefault['deliv_judgment'];
            $arrPC['price02'] = $price02;

            // 該当関数が無いため, セッションの値を直接代入
            $arrPC['creator_id'] = $_SESSION['member_id'];
            $arrPC['updator_id'] = $_SESSION['member_id'];
            ...
            // 2011.8.30 
            // 編集時はproduct_class_idが更新されないようにする
    ...
    function lfCheckProductsClass(&$objFormParam) {
        ...
        if (SC_Utils_Ex::isBlank($arrValues['check'])) {
            $arrErr['check_empty'] = '商品規格を選択してください。<br />';
        }
        ...
        for ($i = 0; $i < $total; $i++) {
            ...
            if (!SC_Utils_Ex::isBlank($arrValues['check'][$i])) {
                 ...
                 * 商品コード重複チェック
                 */
                $product_id = $arrValues['product_id'];
                $product_cd = $arrValues['product_code'][$i];
                if ($this->lfExistsProductCode($product_id, $product_cd) > 0) {
                    $arrErr['product_code'][$i] =
                        '商品コードが重複しています。<br />';
                }
                 ...
                 * 価格の必須チェック
                 */
                if (SC_Utils_Ex::isBlank($arrValues['price01'][$i])) {
                    $arrErr['price01'][$i] =
                        NORMAL_PRICE_TITLE . 'が入力されていません。<br />';
                }
                if (SC_Utils_Ex::isBlank($arrValues['price02'][$i])) {
                    $arrErr['price02'][$i] =
                        SALE_PRICE_TITLE . 'が入力されていません。<br />';
                }
                /*
                 * 在庫数の必須チェック
                 */
    ...
    function doUploadComplete(&$objFormParam) {
        $objImage = new SC_Image_Ex(DOWN_TEMP_REALDIR);
        $arrRealFileName = $objFormParam->getValue('down_realfilename');
        if (is_array($arrRealFileName) == false) return;
...
__EOF__;
    ...
    function getProductsClass($product_id) {
        $objQuery =& SC_Query_Ex::getSingletonInstance();
        $col = "product_code, price01, price02, deliv_judgment, stock, stock_unlimited, sale_limit, sale_minimum_number, teiki_flg, stock_status_name, deliv_fee, point_rate";
