...
/**
 * プロモーションマスタ照会 のページクラス.
 *
 * @package Page
 * @author IQUEVE Co.,Ltd.
 * @version $Id: LC_Page_Admin_Products_PromotionSearch.php 113 2013-12-16 02:22:12Z kaji $
 */
class LC_Page_Admin_Products_PromotionSearch extends LC_Page_Admin_Ex {
    ...
    function init() {
        parent::init();
        $this->tpl_mainpage = 'products/promotion_search.tpl';
        $this->tpl_mainno = 'products';
        $this->tpl_subno = 'promotion_search';
        $this->tpl_pager = 'pager.tpl';
        $this->tpl_maintitle = 'プロモーション管理';
        $this->tpl_subtitle = 'プロモーションマスター';
    ...
    function action() {
        ...
        case 'search':
            ...
            // 検索結果の取得
            $this->arrPromotions = $this->findPromotions
    ...
    function lfInitParam(&$objFormParam) {
        // POSTされる値
        $objFormParam->addParam("プロモーションコード", "promotion_cd",
                                STEXT_LEN, 'a',
                                array("SPTAB_CHECK", "MAX_LENGTH_CHECK"));

        // 検索条件
        $objFormParam->addParam("プロモーション区分", "search_promotion_kbn",
                                INT_LEN, 'n',
                                array("MAX_LENGTH_CHECK"));
        $objFormParam->addParam("有効期間(FROM)", "search_valid_from",
                                STEXT_LEN, 'a',
                                array("SPTAB_CHECK", "MAX_LENGTH_CHECK"));
        $objFormParam->addParam("有効期間(TO)", "search_valid_to",
                                STEXT_LEN, 'a',
                                array("SPTAB_CHECK", "MAX_LENGTH_CHECK"));
    ...
    function buildQuery($key, &$where, &$arrValues, &$objFormParam) {
        ...
        switch ($key) {
        // プロモーション区分
        case 'search_promotion_kbn':
            $tmp_where = "";
            foreach ($objFormParam->getValue($key) as $element) {
                if ($element != "") {
                    if (SC_Utils_Ex::isBlank($tmp_where)) {
                        $tmp_where .= " AND (promotion_kbn = ?";
                    } else {
                        $tmp_where .= " OR promotion_kbn = ?";
                    }
                    $arrValues[] = $element;
                }
            }
        ...
        // 有効期間(FROM)
        case 'search_valid_from':
            $where .= " AND valid_from >= ?";
            $arrValues[] = $objFormParam->getValue($key);
            break;

        // 有効期間(TO)
        case 'search_valid_to':
            $where .= " AND valid_to <= ?";
            $arrValues[] = $objFormParam->getValue($key);
            break;
    ...
    function getNumberOfLines($where, $arrValues) {
        ...
        return $objQuery->count('dtb_promotion', $where, $arrValues);
    }
     ...
     */
    function findPromotions($where, $arrValues, $limit, $offset) {
        $objQuery =& SC_Query_Ex::getSingletonInstance();
