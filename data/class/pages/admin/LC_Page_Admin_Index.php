 ...
 * @version $Id: LC_Page_Admin_Index.php 91 2012-04-11 04:39:04Z hira $
 */
class LC_Page_Admin_Index extends LC_Page_Admin_Ex {
    ...
    function action() {
		...
		// SC_Response_Ex::sendRedirect(ROOT_URLPATH . ADMIN_DIR . "order/");
            }else{
...
EOF;
        ...
        if (SC_Utils_Ex::sfIsMatchHashPassword(
            $pass, $arrData['password'], $arrData['salt'])) {
            return true;
        }
        return false;
    }
...
SELECT update_auth
 ...
 WHERE id = "$authority"
EOF;
        $arrAuthority = $objQuery->getAll($sql);
        $arrAuth = $arrAuthority[0];

        // セッション登録
        $sid = $this->lfSetLoginSession(
            $arrData['member_id'],
            $login_id,
            $arrData['authority'], 
            $arrData['name'],
            $arrData['login_date'],
            $arrAuth['update_auth'],
            $arrAuth['csv_download_auth'],
            $arrAuth['inos_auth'],
	    $arrAuth['critical_menu']
        );
        // ログイン情報記録
        $this->lfSetLoginData(
            $sid, $arrData['member_id'],
            $login_id,
            $arrData['authority'],
            $arrData['login_date']
        );
     ...
     * @param integer $update_auth 更新権限 権限なし:0 権限あり:1
     * @param integer $csv_download_auth CSVダウンロード権限 権限なし:0 権限あり:1
     * @param integer $inos_auth 基幹連携権限 権限なし:0 権限あり:1
     *
     * @return string $sid 設定したセッションのセッションID
     */
    function lfSetLoginSession($member_id, $login_id, $authority, $login_name,
			       $last_login, $update_auth, $csv_download_auth,
			       $inos_auth, $critical_menu) {
        ...
        $objSess->SetSession('update_auth', $update_auth);
        $objSess->SetSession('csv_download_auth', $csv_download_auth);
        $objSess->SetSession('inos_auth', $inos_auth);
        $objSess->SetSession('critical_menu', $critical_menu);
