 ...
 * @package Page
 * @author
 * @version $Id: LC_Page_Admin_Basis_DeliveryInput.php 91 2012-04-11 04:39:04Z hira $
 */
class LC_Page_Admin_Basis_DeliveryInput extends LC_Page_Admin_Ex {
    ...
    function init() {
        ...
        $this->arrProductType = 
            $masterData->getMasterData("mtb_product_type");
        $this->arrPayments = 
    ...
    function lfInitParam($mode, &$objFormParam) {
        ...
        switch ($mode) {
            case 'edit':
                ...
                $objFormParam->addParam("伝票No.確認URL", "confirm_url", SMTEXT_LEN, 'n', array("URL_CHECK", "MAX_LENGTH_CHECK"), "http://");
                ...
                if(INPUT_DELIV_FEE) {
                    for($cnt = 1; $cnt <= DELIVFEE_MAX; $cnt++) {
                        $objFormParam->addParam("配達日数$cnt", "days$cnt", 2, 'n', array("EXIST_CHECK", "MAX_LENGTH_CHECK", "NUM_CHECK"));
                    }
                }
                break;
        ...
        if($arrRet['deliv_id'] != "") {
            ...
            if(INPUT_DELIV_FEE) {
                ...
                for($cnt = 1; $cnt <= DELIVFEE_MAX; $cnt++) {
                    ...
                    if($arrRet[$keyname] != "") {
                        $sqlval['deliv_id'] = $deliv_id;
                        $sqlval['days'] = $arrRet[$keyname_days];
                        $sqlval['pref'] = $cnt;
                        // INSERTの実行
                        $sqlval['fee_id'] = $cnt;
    ...
    function lfGetDelivData(&$objFormParam) {
        ...
        $table = "dtb_delivfee";
        $arrRet = $objQuery->select($col, $table, $where, array($deliv_id));
        $objFormParam->setParamList($arrRet, 'days');
        // 支払方法
        $col = 'payment_id';
