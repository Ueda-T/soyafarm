...
class LC_Page_Admin_Order_View extends LC_Page_Admin_Order_Ex {
    ...
    function init() {
        parent::init();
        $this->tpl_mainpage = 'order/view.tpl';
	...
	$this->lfInitParam($objFormParam);
        ...
        $event_code = $objFormParam->getValue('event_code');
        $this->arrMedia = $this->getMedia($event_code);
    ...
    function lfInitParam(&$objFormParam) {
        ...
        // ポイント機能ON時のみ
        if (USE_POINT !== false) {
            $objFormParam->addParam("使用ポイント", "use_point");
        }
    ...
    function lfInitMultipleParam(&$objFormParam) {
        $objFormParam->addParam("商品規格ID", "multiple_product_class_id");
        ...
        $objFormParam->addParam("単価", "multiple_price");
        $objFormParam->addParam("数量", "multiple_quantity");
        $objFormParam->addParam("配送先住所", "multiple_shipping_id");
    }
...
select
    2 as type
   ,fmh.start_date as send_date
   ,fmh.subject as subject
   ,null as template_id
   ,fmc.send_id as send_id
from
    dtb_follow_mail_customer fmc
inner join
    dtb_follow_mail_history fmh
