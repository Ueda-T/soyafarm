...
class SC_CheckError {
    ...
    function CHECK_WQ_SQ_C($value) {
        if(isset($this->arrErr[$value[1]])) {
            return;
        }
        $this->createParam($value);
        if(strlen($this->arrParam[$value[1]]) != 0 &&
	   preg_match("/[\"\',]/", $this->arrParam[$value[1]])) {
            $this->arrErr[$value[1]] = "※ " . $value[0] . "に「\"」、「'」、「,」は入力できません。<br />";
        }
    }
     ...
     * @return void
     */
    function NO_TAB( $value ) {
        if(isset($this->arrErr[$value[1]])) {
            return;
        }
        $this->createParam($value);
        if(strlen($this->arrParam[$value[1]]) != 0 && preg_match("/[\t\r\n]+/u", $this->arrParam[$value[1]])){
            $this->arrErr[$value[1]] = "※ " . $value[0] . "にタブ、改行は含めないで下さい。<br />";
        }
    }
     ...
     *
     * 数値が0以下ならエラーを返す
     * @param integer $value 数値
     * @return void
     */
    function ZERO_AND_OVER_CHECK($value) {
        ...
        $this->createParam($value);
        // 0よりも大きいか判定
        if ($this->arrParam[$value[1]] < 0 && strlen($this->arrParam[$value[1]]) > 0) {
            $this->arrErr[$value[1]] = "※ " . $value[0] . "は0以上の値で入力してください。<br />";
        }
    }

    function MAX_BYTES_SJIS( $value ) {
        if(isset($this->arrErr[$value[1]])) {
            return;
        }
        $this->createParam($value);
        // バイト数の取得
        if (strlen(mb_convert_encoding($this->arrParam[$value[1]], "SJIS-win", "UTF-8")) > $value[2]) {
            $this->arrErr[$value[1]] = "※ " . $value[0] . "は制限文字数を超えています<br />";
        }
    }
     ...
     * @return void
     */
    function LESS_ONE_CHECK( $value ) {
        if(isset($this->arrErr[$value[1]])) {
            return;
        }
        $this->createParam($value);
        if ( strlen($this->arrParam[$value[1]]) > 0 && $this->arrParam[$value[1]] > 1) {
            $this->arrErr[$value[1]] = "※ " . $value[0] . "は1以下の数字で入力してください。<br />";
        }
    }
     ...
     * @return void
     */
    function NUM_MINUS_CHECK( $value ) {
        if(isset($this->arrErr[$value[1]])) {
            return;
        }
        $this->createParam($value);
        if ( strlen($this->arrParam[$value[1]]) > 0 && $this->arrParam[$value[1]] < 0) {
            $this->arrErr[$value[1]] = "※ " . $value[0] . "は正の数で入力してください。<br />";
        }
    }
    ...
    function IS_TELEPHONE($value) {
        ...
        $this->createParam($value);
        if (strlen($this->arrParam[$value[1]]) > 0 &&
	    !preg_match("/^0\d{9,10}$/", str_replace("-", "", $this->arrParam[$value[1]]))) {

            $this->arrErr[$value[1]] = "※ " . $value[0] . "が不正な番号です。<br />";
        }
    }
     ...
     *     [0]: 項目名<br>
     *     [1]: 電番<br>
     *     [2]: 電話番号総数 (指定なしの場合、TEL_LEN)
     * @return void
     */
    function TEL_CHECK2($value) {
        $telLen = isset($value[2]) ? $value[2] : TEL_LEN;
        ...
        // 合計値チェック
        if (strlen($this->arrParam[$value[1]]) > $telLen) {
            $this->arrErr[$value[1]] .= "※ " . $value[0] . "は" . $telLen . "文字以内で入力してください。<br />";
        }
    }
    ...
    */
    ...
    function KANABLANK_CHECK( $value ) {
        ...
        $this->createParam($value);
        if(strlen($this->arrParam[$value[1]]) > 0 && !preg_match("/^([　 \t\r\n]|[ァ-ヶｦ-ﾟー])+$/u", $this->arrParam[$value[1]])) {
            $this->arrErr[$value[1]] = "※ " . $value[0] . "はカタカナで入力してください。<br />";
        }
    }

    function HANKANASP_CHECK($value) {
        if (isset($this->arrErr[$value[1]])) {
            return;
        }
        $this->createParam($value);
	$s = $value[1];
        if (strlen($this->arrParam[$s]) > 0 &&
	    //	    !preg_match("/^([ｦ-ﾟa-zA-Z0-9]|\\s)+$/u", $this->arrParam[$s])) {
	    // 半角(カナ|英|数|記号)を許容する
	    !preg_match("/^[ -~｡-ﾟ]+$/u", $this->arrParam[$s])) {
            $this->arrErr[$value[1]] = "※ " . $value[0] . "半角で入力してください。<br />";
        }
    }
    ...
    function ZERO_CHECK( $value ) {
        ...
        if($this->arrParam[$value[1]] == "0" ){
            $this->arrErr[$value[1]] = "※ " . $value[0] . "は0以外で入力してください。<br />";
        }
    }
    ...
    function ZIP_CHECK( $value ) {
        ...
        if (!preg_match("/\d{3}-*\d{4}/", $this->arrParam[$value[1]])) {
            $this->arrErr[$value[1]] =  "※ $value[0]の形式に誤りがあります。<br />";
        }
    }
    ...
    function DATE_CHECK($value) {
        ...
        // 日付の形式チェック
        if(!preg_match('|^\d{4}\/\d{2}\/\d{2}$|', $this->arrParam[$value[1]])){
            $this->arrErr[$value[1]] = "※ " . $value[0] . "が正しい形式で入力されていません。<br />";
        }else{
            // 日付の有効性チェック
            list($year, $momth, $day) = preg_split("/\//", $this->arrParam[$value[1]]);
            if(!checkdate($momth, $day, $year)){
                $this->arrErr[$value[1]] = "※ " . $value[0] . "が有効な日付ではありません。<br />";
            }
        }
        return;
    }
     ...
     * @return void
     */
    function DATE_TERM_CHECK($value) {

        $date1 = strtotime($this->arrParam[$value[2]]);
        $date2 = strtotime($this->arrParam[$value[3]]);

        // 判定対象1 > 判定対象2 の場合はエラー
        if ($date2 < $date1) {
            $this->arrErr[$value[2]] = "※ " . $value[0] . "が" . $value[1] ."より未来日付になっています。<br />";
        }
    }
     ...
     * @return void
     */
    function DATE_FUTURE_MONTH_CHECK($value) {

        $date1 = strtotime($this->arrParam[$value[2]]);
        $date2 = strtotime("+" . $value[4] . " month");

        // 判定対象1 > 判定対象2 の場合はエラー
        if ($date2 < $date1) {
            $this->arrErr[$value[2]] = "※ " . $value[0] . "は" . $value[4] ."ヶ月以内の日付を指定してください。<br />";
        }
    }
}
?>
