...
class SC_FormParam {
     ...
     * チェック項目を取り除く.
     *
     * @param  array $arrCol :対象カラム配列
     * @param  string $check :チェック項目
     */
    function removeCheck($arrCol, $check) {

        // 取り除きたいカラムをループ
        foreach ($arrCol as $col) {

            foreach ($this->keyname as $key => $colName) {
                // 対象カラム以外スキップ
                if ($colName != $col) {
                    continue;
                }
                // チェック項目をループ
                foreach ($this->arrCheck[$key] as $k => &$err) {
                    if ($err != $check) {
                        continue;
                    }
                    // チェック項目を消去
                    unset($this->arrCheck[$key][$k]);
                    sort($this->arrCheck[$key]);
                }
            }
        }
    }
     ...
     * @param  string $check  :チェック項目
     */
    function addCheck($arrCol, $check) {
        ...
        foreach ($arrCol as $col) {
            ...
            foreach ($this->keyname as $key => $colName) {
                ...
                // チェック項目を追加
                $this->arrCheck[$key][] = $check;
                // 重複制御
                $this->arrCheck[$key] =
                    array_unique($this->arrCheck[$key]);
                sort($this->arrCheck[$key]);
            }
        }
    }
