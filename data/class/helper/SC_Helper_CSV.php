...
$where
ORDER BY dtb_order.order_id,dtb_order_detail.order_detail_id
EOF;
...
SELECT
    $cols
FROM
    dtb_review AS A INNER JOIN dtb_products AS B
...
EOF;
	...
	GC_Utils::gfFrontLog(print_r($data, true));

	if ($this->csvID == '3') {
	    if ($this->prevOrderID != $data['order_id']) {
		$this->prevDetailNo = 0;
	    }
	    $data['order_detail_id'] = ++$this->prevDetailNo;
	    $this->prevOrderID = $data['order_id'];
	}
     ...
     * @param boolean $is_download true:ダウンロード用出力までさせる false:CSVの内容を返す(旧方式、メモリを食います。）
     */
    function sfDownloadMakeCsvFromSql($sql, $arrVal = array(), $file_head = 'csv', $arrHeader = array()) {
        $objQuery =& SC_Query_Ex::getSingletonInstance();
