...
FROM
    dtb_category as cat
    left join dtb_category_total_count as ttl on ttl.category_id = cat.category_id
WHERE
    $where
ORDER BY rank DESC
EOF;
...
SELECT
    ...
    ttl.product_count
FROM
    dtb_category as cat left join dtb_category_total_count as ttl
        on ttl.category_id = cat.category_id
WHERE
    $where
ORDER BY rank DESC
EOF;
...
WHERE
    category_id = ?
EOF;
        $res = $objQuery->getAll($sql, $arrVal);
...
__EOS__;
    ...
    function sfGetBrandWhere($brand_id) {
        // 子ブランドIDの取得
        $arrRet = SC_Helper_DB_Ex::sfGetChildrenArray("dtb_brand", "parent_id", "brand_id", $brand_id);

        $where = "brand_id IN (" . implode(',', array_fill(0, count($arrRet), '?')) . ")";
...
__EOS__;
    ...
    function extractChilds($parent_id, $childs) {
        for ($i = 0; $i < count($childs); ++$i) {
	    if ($parent_id == $childs[$i]['parent_id']) {
		$r[] = $childs[$i];
	    }
	}
	return $r;
    ...
    function sfGetBrandList($head = BRAND_HEAD) {
        $objQuery =& SC_Query_Ex::getSingletonInstance();
	$sql =<<<__EOS
SELECT
    brand_id,
...
WHERE
    del_flg = 0
AND now() BETWEEN disp_start_date AND disp_end_date
AND parent_id IS %s NULL
ORDER BY
    rank ASC
__EOS;

        $parents = $objQuery->getAll(sprintf($sql, ""));
        $childs = $objQuery->getAll(sprintf($sql, "NOT"));

        for ($i = 0; $i < count($parents); ++$i) {
            $arrAll[] = $parents[$i];
	    $r = $this->extractChilds($parents[$i]['brand_id'], $childs);
	    for ($j = 0; $j < count($r); ++$j) {
		$arrAll[] = $r[$j];
	    }
	}
        ...
        for($i = 0; $i < count($arrAll); ++$i) {
            $id = $arrAll[$i]['brand_id'];
            $name = $arrAll[$i]['brand_name'];
	    $level = isset($arrAll[$i]['parent_id']) ? 1 : 0;
            $arrList[$id] = str_repeat($head, $level) . $name;
	}
    ...
    /**
     * 最終出力日時を取得する(INOSシステム連携)
     *
     * @param  int $data_type データ種別(1:顧客、2:受注、3:定期)
     * @return 最終出力日時
     */
    function sfGetLastSendDate($data_type) {
...
__EOS;
     ...
     * 
     * @param  int $data_type データ種別(1:顧客、2:受注、3:定期)
     * @param  int $send_date 出力日時
     * @return void
     */ 
    function sfUpdateLastSendDate($data_type, $send_date) {
...
UPDATE
    dtb_batch_control
SET
    last_send_date = '$send_date',
    updator_id  =    '$member_id',
    update_date =    now()
WHERE
    data_type = $data_type
AND del_flg = '0'
__EOS;
     ...
     * バッチ処理履歴情報を登録する
     *
     * @param  int $data_type     データ種別
     * @param  int $process_count 処理件数
     * @param  int $process_flg   処理フラグ
     * @param  void
     */
    function sfInsertBatchHistory($data_type, $process_count, $process_flg) {
...
__EOS;
    ...
    /**
     * 郵便番号からmtb_zipのカナを取得する
     *
     * @param  $zipcode 郵便番号
     * @return 住所カナ
     */
    function sfGetAddrKanaByZipcode($zipcode = "") {
        ...
        $sql =<<<__EOS
SELECT
    CONCAT(state_kana, city_kana, town_kana) AS addr_kana
FROM
    mtb_zip
WHERE
    zipcode = '{$zipcode}'
__EOS;
    ...
    /**
     * #228
     * mtb_statusからステータス情報一覧を取得する.
     *
     * @return array [id] => [image] の配列
     */
    function sfGetStatusImageList() {
        $objQuery =& SC_Query_Ex::getSingletonInstance();
...
ORDER BY rank
__EOS;
        $arrImage = $objQuery->getAll($sql);
        $count = count($arrImage);
        for($cnt = 0; $cnt < $count; $cnt++) {
            $key = $arrImage[$cnt]["id"];
            $val = $arrImage[$cnt]["image"];
            $arrRet[$key] = $val;
        }
        return $arrRet;
    }
     ...
     * mtb_statusからステータス情報一覧を取得する.
     *
     * @return array 商品ステータス情報の配列
     */
    function sfGetProductStatusList() {
        $objQuery =& SC_Query_Ex::getSingletonInstance();
...
ORDER BY rank
__EOS;
        $arrStatus = $objQuery->getAll($sql);
        $count = count($arrStatus);
        for($cnt = 0; $cnt < $count; $cnt++) {
            $key = $arrStatus[$cnt]["id"];
            $name = $arrStatus[$cnt]["name"];
            $image = $arrStatus[$cnt]["image"];
            $arrRet[$key]["name"] = $name;
            $arrRet[$key]["image"] = $image;
        }
        return $arrRet;
    }

    /**
     * 基幹端末種別IDをキーとした端末種別IDの配列を取得
     *
     * @param none
     * @return array 基幹端末種別ID一覧
     */
    function sfGetDeviceTypeId() {
        $objQuery =& SC_Query_Ex::getSingletonInstance();
...
ORDER BY rank
__EOS;
        $arrDevice = $objQuery->getAll($sql);
        $count = count($arrDevice);
        for($cnt = 0; $cnt < $count; $cnt++) {
            $key = $arrDevice[$cnt]["kikan_id"];
            $device_type_id = $arrDevice[$cnt]["id"];
            $arrRet[$key] = $device_type_id;
        }
        return $arrRet;
    }

    /**
     * 指定したブランドIDのブランドを取得する.
     *
     * @param integer $brand_id ブランドID
     * @return array 指定したブランドIDのブランド
     */
    function sfGetBrand($brand_id){
        $objQuery =& SC_Query_Ex::getSingletonInstance();

        // ブランドを取得する
        $arrVal = array($brand_id);
