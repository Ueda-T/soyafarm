 ...
 * @version $Id: SC_Product.php 103 2012-04-24 02:08:48Z hira $
 */
class SC_Product {
    ...
    var $arrOrderData;
    // 2013.11.29 在庫切れ時の表示
    var $stock_status_name;
    /** 数量 */
    var $arrQuantity = array();
...
LEFT JOIN dtb_brand_products AS BP
    ON T1.product_code = BP.product_code
__EOS__;
...
__EOS__;
    ...
    function lists(&$objQuery, $arrVal = array()) {
        $col = <<< __EOS__
            ...
            ,sale_limit_max
            ,sale_limit_min
            ,sale_minimum_number_max
            ,sale_minimum_number_min
            ,deliv_date_id
            ,status
            ,del_flg
            ,update_date
...
__EOS__;
    ...
    function getDetail($productId) {
	    ...
	    ("*", $this->alldtlSQL("product_id = ? AND del_flg = 0"),
	     "product_id = ?", array($productId, $productId));
    ...
    function setProductsClassByProductIds($arrProductId, $has_deleted = false) {
        ...
        foreach ($arrProductId as $productId) {
            ...
            foreach ($arrProductClass as $productsClass) {
                ...
                // 在庫切れ時の表示文言
                if ($stock_find_class === false) {
                    if ( !empty($productsClass['stock_status_name']) ) { 
                        $stock_status_name = $productsClass['stock_status_name'];
                    } else {
                        $stock_status_name = ' 品切れ中';
                    }
                    $this->stock_status_name[$productsClass['product_class_id']] =
                        $stock_status_name;
                }

                // 数量
                $sale_limit = $productsClass['sale_limit'];
                $sale_minimum_number = $productsClass['sale_minimum_number'];

                $arrQuantity = array();
                for ($i=$sale_minimum_number; $i<=$sale_limit; $i++) {
                    $arrQuantity[$i] = $i;
                }
                $this->arrQuantity[$productsClass['product_class_id']] = $arrQuantity;

                // 定期フラグ
                $this->teiki_flg[$productsClass['product_class_id']] = $productsClass['teiki_flg'];

                $classCats2['classcategory_id2'] = $productsClass2;
                //$classCats2['name'] = $productsClass['classcategory_name2'] . ($stock_find_class ? '' : ' (品切れ中)');
                $classCats2['name'] = $productsClass['classcategory_name2'] . ($stock_find_class ? '' : '');
                ...
                if (!in_array($classcat_id1, $classCats1)) {
                    $classCats1[$productsClass1] = $productsClass['classcategory_name1']
                        ...
                        . ($productsClass2 == 0 && !$stock_find_class ? '' : '');
                }
                ...
                if ($customer_kbn == CUSTOMER_KBN_EMPLOYEE) {
                    $price = $productsClass['price02'];
                // 社員以外
                } else {
                    $price = $productsClass['price01'];
                }
                $classCats2['point']
                    = number_format(SC_Utils_Ex::sfPrePoint($price, $productsClass['point_rate']));
    ...
    function getProductsClassByQuery(&$objQuery, $params) {
        ...
        $col = <<< __EOS__
            ...
            T1.stock,
            T1.stock_unlimited,
            T1.stock_status_name,
            T1.teiki_flg,
            T1.deliv_judgment,
            T1.sale_limit,
            T1.sale_minimum_number,
            T1.price01,
            T1.price02,
            T1.point_rate,
            T1.product_code,
...
__EOS__;
     ...
     *
     * @param array $productIds 商品ID の配列
     * @param integer $deviceTypeId 端末種別ID
     * @return array 商品IDをキーにした商品ステータスIDの配列
     */
    function getProductStatus($productIds, $deviceTypeId = DEVICE_TYPE_PC) {
        if (empty($productIds)) {
            return array();
        }
        $objQuery =& SC_Query_Ex::getSingletonInstance();

         $cols = 'PS.product_id, PS.product_status_id';
        $from = 'dtb_product_status PS INNER JOIN mtb_status MS ON PS.product_status_id = MS.id';
        $where = 'PS.del_flg = 0 AND PS.product_id IN (' . implode(', ', array_pad(array(), count($productIds), '?')) . ') AND PS.device_type_id = ' . $deviceTypeId;
        $order = "MS.rank";
        $objQuery->setOrder($order);
    ...
    function setProductStatus($productId, $productStatusIds,
                              $deviceTypeId = DEVICE_TYPE_PC) {
        $val['product_id'] = $productId;
        $val['device_type_id'] = $deviceTypeId;
        $val['creator_id'] = $_SESSION['member_id'];
        $val['create_date'] = 'Now()';
        $val['updator_id'] = $_SESSION['member_id'];
        $val['update_date'] = 'Now()';
        $val['del_flg'] = '0';

        $objQuery =& SC_Query_Ex::getSingletonInstance();
        $objQuery->delete('dtb_product_status',
                          'product_id = ? AND device_type_id = ?',
                          array($productId, $deviceTypeId));
    ...
    /**
     * 商品詳細の結果から, 購入最低数を取得する.
     *
     * getDetailAndProductsClass() の結果から, 最低購入数を取得する.
     *
     * @param array $p 商品詳細の検索結果の配列
     * @return integer 商品詳細の結果から求めた最低購入数.
     * @see getDetailAndProductsClass()
     */
    function getBuyMinimumNumber($p) {
        $limit = null;
        if ($p['stock_unlimited'] != '1' && is_numeric($p['sale_minimum_number'])) {
            $limit = min($p['sale_minimum_number'], $p['stock']);
        } elseif (is_numeric($p['sale_minimum_number'])) {
            $limit = $p['sale_minimum_number'];
        } elseif ($p['stock_unlimited'] != '1') {
            $limit = $p['stock'];
        }
        return $limit;
     ...
     */
    function alldtlSQL($where = "", $alias = "alldtl") {
        ...
        $sql = <<< __EOS__
            (
                SELECT 0
                    ...
                    ,dtb_products.name
		    ,dtb_products.disp_name
                    ,dtb_products.maker_id
                    ,dtb_products.status
                    ,dtb_products.sales_name
                    ,dtb_products.brand_kbn
                    ...
                    ,dtb_products.note
                    ,dtb_products.pc_comment1
                    ,dtb_products.pc_comment2
                    ,dtb_products.pc_comment3
                    ,dtb_products.pc_comment4
                    ,dtb_products.pc_comment5
                    ,dtb_products.pc_button4
                    ,dtb_products.pc_button5
                    ,dtb_products.sp_comment1
                    ,dtb_products.sp_comment2
                    ,dtb_products.sp_comment3
                    ,dtb_products.sp_comment4
                    ,dtb_products.sp_comment5
                    ,dtb_products.sp_button4
                    ,dtb_products.sp_button5
                    ,dtb_products.mb_comment1
                    ,dtb_products.mb_comment2
                    ,dtb_products.mb_comment3
                    ,dtb_products.mb_comment4
                    ,dtb_products.mb_comment5
                    ...
                    ,dtb_products.capacity
                    ,dtb_products.sale_start_date
                    ,dtb_products.sale_end_date
                    ,dtb_products.guide_image
                    ,dtb_products.guide_image_teiki
                    ,dtb_products.metatag
                    ,dtb_products.deliv_kbn1
                    ,dtb_products.deliv_kbn2
                    ,dtb_products.drop_shipment
                    ,dtb_products.component_flg
                    ,dtb_products.employee_sale_cd
                    ,dtb_products.del_flg
                    ,dtb_products.creator_id
                    ,dtb_products.disp_start_date
                    ...
                    ,T4.sale_limit_max
                    ,T4.sale_limit_min
                    ,T4.sale_minimum_number_max
                    ,T4.sale_minimum_number_min
                    ,T4.point_rate
                    ,T4.deliv_fee
                    ,T4.deliv_fee_quantity
                    ,T4.class_count
                    ,dtb_maker.name AS maker_name
                    ,dtb_brand.brand_name
                    ,T4.teiki_flg
                FROM dtb_products
                    JOIN (
                       SELECT product_id,
...
__EOS__;
     ...
     * @return array 商品規格情報の配列
     */
    function getProductsClassProductCode($productCode) {
        $objQuery =& SC_Query_Ex::getSingletonInstance();

		$col = "product_class_id, product_id";
		$where = "del_flg = 0 AND product_code = ?";

		$arrRes = $objQuery->select($col, "dtb_products_class", $where
									, array($productCode));
     ...
     * @return 販売対象フラグ
     */
    function getSellFlgByProductCode($productCode) {
        $objQuery =& SC_Query_Ex::getSingletonInstance();
...
__EOS;
    ...
    /**
     * 商品規格IDを指定して、該当の商品が定期可能かチェックする
     *
     * @param integer $product_class_id 商品規格ID
     * @return boolean true:定期可 false:定期不可
     */
    function checkTeikiFlg($product_class_id) {
...
__EOS;
        $teiki_flg = $objQuery->getOne($sql);

        // 定期可
        if ($teiki_flg == REGULAR_PURCHASE_FLG_ON) {
            return true;
        } else {
            return false;
        }
     ...
     * @return 商品規格ID
     */
    function getProductClassIdByProductId($product_id) {
        $objQuery =& SC_Query_Ex::getSingletonInstance();
