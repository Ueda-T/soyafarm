<?php
/**
 * LC_Page_SBIVT3G_SuicaApp.php - LC_Page_SBIVT3G_SuicaApp クラスを定義
 *
 * LICENSE: （ライセンスに関する情報）
 *
 * @category    Veritrans
 ...
 * @version     $Id: LC_Page_SBIVT3G_SuicaApp.php 185 2012-07-30 07:20:45Z hira $
 ...
 * @author  K.Hiranuma
 */
class LC_Page_SBIVT3G_SuicaApp extends LC_Page_SBIVT3G {
    ...
    function suicaAppExecute() {
        ...
        if ($this->method == 'MOBILE') {
            $optionType = MDL_SBIVT3G_EM_TYPE_MOBILE_SUICA_APP;
        } else {
            $optionType = MDL_SBIVT3G_EM_TYPE_PC_SUICA_APP;
        }
        ...
        // 実行
        $logger->info(sprintf('Suica決済(%s:アプリ型)通信実行', $this->method));
    ...
    function goToComplete() {
        ...
        // memo03:ログ情報
        if ($this->method == 'MOBILE') {
            $innerPay = MDL_SBIVT3G_INNER_ID_SUICA_MOBILE_APP;
        } else {
            $innerPay = MDL_SBIVT3G_INNER_ID_SUICA_PC_APP;
        }
        $other = '成功';
    ...
    function genPaymentButton($url) {
        ...
        if ($this->method == 'MOBILE') {
            ...
            $html = <<<EOD
<div align="center">
  <a href="$url"><font size="+1">モバイルSuica決済を開始</font></a>
</div>
...
<a href="$url" target="_brank" style="display:block;margin:10px;font-size:120%;">Suica Internet Serviceでの決済を開始する</a>
